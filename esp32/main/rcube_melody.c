 ...
 * 기획서 5장 [소리 규칙]의 "연결대기 멜로디"이나 xlsx에 음 정의가 없어 임의로 지었다.
 * ★ 2026-07-30부터 호출부가 없다. 버튼음(미도파도) 직후에 이어 붙으면 여섯 음이 되어
 *   LINK(레도미도파도)와 구분되지 않는다는 청취 결과 때문이다. 연결모드 진입은 버튼음
 *   하나로 알리고, 연결음은 상위가 실제로 붙는 시점에 LINK로 낸다.
 *   음 정의가 확정되면 되살릴 수 있게 데이터는 남겨 둔다. */
